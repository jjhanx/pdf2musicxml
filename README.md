...
| `POST /api/review/:jobId` | 본문은 **항목 배열**이거나 `{ "items": [...], "transposeSemitones"?: number }` — 마스킹/가사 분류 제출, 선택 시 **곡 전체 동일** 반음 이동(`ocr_meta.json` 반영) 후 Audiveris 단계 재개. 부분만 틀린 음높이에는 부적합(아래 품질 가이드 참고) |
