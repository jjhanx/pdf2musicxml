...
- **OMR 리듬·화음 slur 가드 강화**: `fix_audiveris_mxl.py` — 세잇단 run 직전 4분 화음 펼침·plain 8분 3개 세잇단화는 **voice가 마디 길이를 정확히 넘칠 때만** 실행(이미 맞는 마디 부작용 방지). **2성부 화음 slur**는 아래 성부 `below`·위 성부 `above` placement. 마디 **맨 앞** 연속 4분 2개→빔 8분 쌍 복원 추가. 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
