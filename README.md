...
- **OMR 엔진 기본값 AI + TrOMR**: `OMR_ENGINE` 미설정 시 **AI OMR(TrOMR)** 이 동작합니다. `pip install -r requirements-ai.txt` 필요. Audiveris·Java **불필요**.
...
- **OMR 리듬 후처리 기본값 변경**: `fix_audiveris_mxl.py`는 **`AUDIVERIS_MXL_RHYTHM_FIX=off`(기본)** 로 Audiveris **리듬·duration을 그대로** 둡니다. 4분↔8분 추정·끝 𝄽8 삽입·세잇단 펼침은 **`legacy`** 모드에서만. ♩↔♪ 오류는 **OMR 품질 검토(HITL)**·`mxl-lint` overfull/underfull 목록으로 수정. [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
...
pip install -r requirements-ai.txt   # TrOMR (torch, transformers)
```
...
| `AI_OMR_BACKEND` | **`tromr`(기본, TrOMR-large)** \| `mock`(개발용). `pip install -r requirements-ai.txt` |
