...
- **같은 PDF 반복 작업 — 시작 단계 선택**: 변환 전 **「변환 시작 단계 선택」**에서 ① **1단계** 원본 PDF, ② **clean_score_only.pdf + 분리된 가사 JSON**, ③ **omr-work.zip**(가사 포함), ④ **omr-work.zip**(교정 완료 MXL) + **가사 JSON** 으로 이어갈 수 있습니다. 2단계부터는 **가사 manifest 없이 완성 MXL을 만들 수 없으므로** 분리 가사를 함께 올립니다. 작업 목록에 **단계별 진행**(OMR·HITL·가사 검증 대기 포함)이 표시됩니다. [docs/일반_품질_및_HITL_로드맵.md](docs/일반_품질_및_HITL_로드맵.md).
