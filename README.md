...
- **Audiveris 후처리 (최근)**: 세잇단 **쉼표 없음→`show-bracket=no`(숫자만)**, **쉼표 포함→bracket 유지**. **조바꿈 마디 F 조표 오인** — 한 파트라도 `<key>` 조바꿈이 있는 마디에서 다른 성부의 F `<clef>`만 있으면 제거하고 `<key>` 보충(`fix_audiveris_mxl` + OSMD 미리보기, 마디·파트 하드코딩 없음). 화음 멤버 beam 일괄 제거·마디 임시표 역전파·m50 전용 accidental 패치는 **제거**(범용 오류 유발).
