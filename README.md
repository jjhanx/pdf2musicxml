...
- **OSMD 미리보기 박자 간격**: Softmax·PR/PL 시스템 정렬로 박자 간격이 들쑥날쑥해 보이던 문제를, render 후 **onset 비례 `data-osmd-layout-x` → SVG** 재배치로 보정합니다(PR≠PL part 매칭, PL staffIdx·base part shared span). 저장 MXL 불변. SoftmaxFactor≥100 보조. 회귀: `npx tsx _smoke/test_onset_layout_beat_spacing.ts`, `npx tsx _smoke/test_pr_pl_onset_beat_spacing.ts`.
...
- **변환 방식 선택(웹 UI)**: 업로드 전 **변환 방식**을 고릅니다.
  ...
  - **OMR만**: 선행 처리·가사 주입 없음.
- **OMR 품질 검토(HITL)**: `mxl-lint`의 **「OMR 불일치 의심 마디」자동 목록은 UI에서 제거**했습니다(오탐이 많음). **PDF·MusicXML 나란히 대조** 후 **마디 클릭 → 해당 마디만 편집** 흐름은 그대로입니다. 마디 편집에서 **세잇단(잇단) 적용·해제**·**빔(연결줄) 연결·해제**·**음표별 direction**(`addNoteDirection`/`setNoteDirection`/`clearNoteDirection` — 셈여림·words·rehearsal, **각 음표·쉼표** 행에서 「direction 추가」, **같은 `#n`에 a tempo+ff 등 복수** 가능, 목록 `dir:mf`/`txt:…` — **P2 파트 ID ≠ staff 2**, PL 필터 **낮은음자리표(F) 유지**)·**「동시 시작 voice 복원」**(화음 아닌 동시·다른 박자 — **전체·PR·PL 모두**, 2단 part는 staff 선택)·**표(articulation) 추가·제거·위/아래 위치**(tenuto·accent 등)·**이음줄(slur) 추가·제거·위/아래 위치·오선 거리(1-10칸)**·**꾸밈음(ornament) 추가·제거**(원본에 없는 inverted-mordent 지그재그 등 `<ornaments>`)·**셈여림 점선(wedge)** 시작→끝 추가·`wedge(stop)`을 **끝 음 뒤**(barline·다음 마디 아님)에 두어 길이 조정·**꾸밈음(grace) 추가·삭제**·**늘임표(fermata) 추가·제거**·**붙임줄(tie) 연결** — 같은 마디 `#n` + **마디 넘김**(다음/이전 MXL m·음높이)·**「여기 뒤」음표·쉼표 삽입**(점음표·점쉼표 포함)·**화음 음 추가**·**화음 멤버는 그 음만 삭제**(같은 화음 전체가 같이 지워지지 않음)·**같은 피치 화음 중복은 하나만 남김**·**음높이·삽입 음표의 임시표**(♯/♭/♮·**♯♯/♭♭**) 를 지원합니다. **성부 필터**는 피아노 `P`를 2줄이면 **PR/PL**로 나누고, MXL 미리보기·마디 편집 모두 해당 줄만 표시합니다(병렬 voice `<backup>` 유지). **전체 악보 OSMD 미리보기**는 2단 grand staff part를 **PR·PL 단일 줄 part로 분리**합니다(저장 MXL은 그대로). **OSMD 미리보기 전용** `repairTimelineForOsmdPreview`로 Audiveris **orphan `<backup>`/`<forward>`** 와 **`<print new-page>`·`<print new-system>`** 을 제거합니다(페이지·시스템 직후 마디가 OSMD에서 밀리거나 비는 **범용** 패턴 — 저장 MXL 불변). 배포 반영 후 **`npm run build`·서버 재시작·Ctrl+F5** 필요. **voice별 cursor**로 박자 시각을 계산하고, **순차(비겹침)** 다중 voice를 단일 voice + `<forward>`로 평탄화해 PR·PL **박자 정렬**을 맞춥니다(HITL **음표 삽입·박자 수정** 후 stale backup이 있어도). **「MXL에 반영·미리보기」** 시 timeline 재정렬이 `<forward>` 앞 backup duration을 앞 voice 길이에 맞춥니다. direction은 **part·마디·음표 `#n`** + anchor `voice`·`default-x`(저장 MXL). OSMD에서 PL direction이 P2·P1 등에 어긋나 보이면 **PL 필터**로 해당 줄만 확인. **part-name·instrument-name**은 Audiveris `Voice`/`Piano` 대신 **`part_labels.json` 확정 라벨**(S/A/T/B/PR/PL)을 씁니다(라벨 미확정 시 preset·P1 추정이 잠깐 보일 수 있음). 빔 연결 시 **voice·staff·stem·duration**을 맞추고, **`<beam>`은 화음 리더에만** 기록합니다(`<chord/>` 멤버 beam은 OSMD 미리보기 누락 원인 — `cleanup_chord_beams_mxl.py`로 자동 제거). 삽입 음표는 **`default-x`를 이웃에서 상속**해 반영 후 마디 맨 앞으로 밀리지 않습니다. **「MXL에 반영·미리보기」** 후 편집 중인 마디가 **미리보기 스크롤 영역 세로 중앙**에 오도록 자동 스크롤하며, 반영된 보정은 대기 목록에서 제거되고 baseline MXL에 저장되어, 이후 보정은 누적 재적용 없이 incremental로 반영됩니다. **세잇단 placement**는 `fix_audiveris_mxl.py`와 동일하게 **stem down→below, stem up→above**(쉼표 시작 시 구간 음표 stem 기준). 자세한 절차·문제 해결: [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md) §OMR 품질 검토.
- **마디 끝 예고 음자리표 삭제**: 다음 마디 머리 clef가 OSMD에서 이전 마디 끝 courtesy처럼 보이는 경우, 이전 마디 편집기 목록에 **“다음 m.N 머리 예고”** 로 표시됩니다. 여기서 삭제하면 실제 XML 위치인 다음 마디 head clef만 제거됩니다.
- **마디 앞 음자리표 보정**: 마디 편집에서 **마디 앞**에 𝄞/𝄢를 넣거나 현재 마디 clef를 바꾸면, PL처럼 `<backup>` 뒤에 첫 staff 음표가 있는 경우도 **현재 마디 시작 clef**로 정리하고 기존 시작 clef를 제거/갱신합니다. 이전 마디 끝 예고표만 생기고 현재 마디 음이 여전히 옛 clef로 그려지는 문제를 막습니다.
...
- **웹 UI**: PDF 파일 선택(복수), **드래그 앤 드롭**(전용 영역), 직관적인 커스텀 파일 찾기 버튼, 일괄 변환(순차 처리), 파일별 진행 표·개별 다운로드(실패 행 **결과** 열에는 서버 오류 메시지 전체를 줄바꿈으로 표시). 각 단계별 필요 설정(OMR 직후 멈춤 등)은 해당 단계 폼 내부에 명확하게 배치되어 있습니다.
- **진행 표시**: 업로드 단계와 **Audiveris** OMR 단계의 진행 상황을 표시합니다. OMR 품질 검토 **「작업 저장(ZIP)」** 도 퍼센트(%)로 진행을 보여 줍니다.
...
- **Audiveris 직후 OMR 품질 검토(HITL, 기본 켜짐)**: 성부 라벨 확정 후 **PDF·MusicXML(OSMD) 나란히 미리보기**로 대조하고, **오른쪽 악보에서 마디를 클릭**(오선·음표 영역, 호버 시 파란 표시)해 점(·)·쉼표·음표·**세잇단**·**빔**·**마디 템포(BPM)**·**연주순번(음표·쉼표)** 등을 요소별 보정 → **MXL에 반영·미리보기**로 OSMD에서 확인 → **이어하기**로 최종 MXL까지 반영합니다(MuseScore 불필요). **이미지 PDF(`image_pdf`)** 검토는 페이지 넘김 부담을 줄이기 위해 OSMD를 **성부+선택 마디 1개**만 그리고 PNG를 저해상도·긴 변 제한으로 받으며, **마디 번호 / ◀마디 / 마디▶**로 PDF 페이지까지 같이 이동합니다. **템포 추가 후 첫 마디가 비거나 음표가 사라지면** OSMD가 `<sound tempo>`만 있는 direction을 pickup으로 해석하는 버그입니다 — 모든 파트에 metronome을 두고 첫 파트만 표시(`print-object="no"`)하도록 수정됨. **작업 저장(ZIP)/불러오기**로 검토를 중단·재개할 수 있으며(ZIP에 **clean_score·원본 PDF·lyric_manifest.json** 포함), **3단계**에서 **omr-work.zip**으로 Audiveris 없이 이어가거나, **4단계**에서 교정 완료 ZIP + 가사 JSON으로 가사 검증·주입만 진행할 수 있습니다. [docs/일반_품질_및_HITL_로드맵.md](docs/일반_품질_및_HITL_로드맵.md), [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md) §OMR 품질 검토 · §이미지 PDF HITL.
...
- **결과 자동으로 다운로드(저장)**: UI에서 "결과 자동으로 다운로드(저장)하기"를 체크하면 변환이 완료된 후 별도로 다운로드 버튼을 누르지 않아도 자동으로 `.mxl` 파일이 다운로드됩니다. 파일명은 업로드한 **원본 PDF 이름**(예: `곡제목.mxl`)을 기본으로 하며, 2단계만 이어할 때는 `*-clean-score-only.pdf` 이름에서 접미사를 뺀 값을 씁니다.
...
| `POST /api/omr-hitl/:jobId/export-work/start` | OMR 검토 ZIP 백그라운드 생성 시작(진행률 %) |
