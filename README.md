...
- **OMR 리듬·tie 자동 보정 확대**: `fix_audiveris_mxl.py` 범용 보정 — ① 세잇단 **쉼표 없음→숫자 '3'만**, **쉼표 포함→bracket**, ② **𝄽8+8분 2개** 세잇단·점4분→8분 오인, ③ **E4+G4 연속 8분 화음 slur**(음머리 쪽 placement)·**동음 연장 slur(동일 `<type>`일 때만)**, ④ **세잇단 run 직전 4분 화음→세잇단 3slice** 복원(voice overfull일 때), ⑤ 마디 **첫 화음** `#` 오인 natural→sharp(**duplicate pitch** 시 재배치), ⑥ 잇단 `normal-type` 보강(OSMD), ⑦ 쉼표 전용 voice 병렬 유지, ⑧ **피아노 RH** — voice flatten 생략 시 **staff `default-x` 타임라인**으로 ♩. 뒤 4분→8분·앞 ♩♩+빔8분→♪♪(다른 voice ♩.+(♪|𝄽8) 힌트 포함), ⑨ **세잇단+일반 8분 혼재 voice** overfull 세잇단화 생략. **임시표 역전파 비활성**. 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
