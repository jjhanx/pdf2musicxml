...
- **Audiveris 직후 OMR 품질 검토(HITL, 기본 켜짐)**: Audiveris가 MXL을 만든 직후 **`mxl_quality_lint.py`** 로 P류 direction·마디 끝 쉼표·마디 경계 순서 등을 점검하고, **페이지×성부** UI에서 PDF와 대조한 뒤 **이어하기**로 가사 주입 단계로 넘깁니다. 단계별 사용자 작업·API는 [docs/일반_품질_및_HITL_로드맵.md](docs/일반_품질_및_HITL_로드맵.md).
