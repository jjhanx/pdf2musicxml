...
- **OMR 리듬·tie 자동 보정 확대**: `fix_audiveris_mxl.py` 범용 보정 — … ⑨ **세잇단+일반 8분 혼재 voice** overfull 세잇단화 생략, ⑩ **PL orphan plain 빔 8분 3개→1절 세잇단**, **빔 run 뒤 4분 화음→8분 2개**, **♩. 보정 후 voice default-x 재정렬**. **임시표 역전파 비활성**. 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
