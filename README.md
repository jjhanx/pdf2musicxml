...
- **Audiveris 후처리 (최근)**: 세잇단 **쉼표 없음→`show-bracket=no`(숫자만)**, **쉼표 포함→bracket 유지**. **PL 미리보기 clef** — grand staff 줄바꿈 마디에 PR G clef만 있을 때 PL에 G가 씌워지던 OSMD split 버그 수정(저장 MXL 불변). **HITL verbatim split** — PR·PL part마다 `<staves>1</staves>`·clef·note staff 정규화(7줄·PR/PL 라벨 중복·PL 음높이 어긋남 방지, 저장 MXL 불변). **조바꿈 F clef 오인 미리보기(일반)** — Audiveris가 `<key>` 대신 `<clef>F</clef>`만 내보내는 SATB·줄바꿈 마디에서 **직전 G clef + median pitch≥E3** treble 오인만 제거·`<key>` 보충, **피아노 bass F clef 등 유효 F clef 유지**(MusicXML pitch 절대값 — octave transpose 없음, 저장 MXL 불변). HITL verbatim은 m1 조표 주입·courtesy clef 정리 생략. **HITL 전 `fix_audiveris_mxl` 미적용**. **최종 MXL·ZIP 다운로드** 기본명은 원본 PDF(`sourcePdfDisplayName`), `input.pdf`·`upload_clean_score.pdf` 등 세션 내부명 제외. 화음 멤버 beam 일괄 제거 등 **제거**(범용 오류).
