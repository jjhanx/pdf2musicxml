...
- **Audiveris 직후 OMR 품질 검토(HITL, 기본 켜짐)**: 성부 라벨 확정 후 **PDF·MusicXML(OSMD) 나란히 미리보기**로 대조하고, **MXL 악보에서 마디를 클릭**해 쉼표 줄·점(·)·음높이·박자·이음줄·direction 등을 요소별로 보정 → **보정 MXL에 적용** → **이어하기**로 최종 MXL까지 반영합니다(MuseScore 불필요). [docs/일반_품질_및_HITL_로드맵.md](docs/일반_품질_및_HITL_로드맵.md).
