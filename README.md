...
- **Audiveris 직후 OMR 품질 검토(HITL, 기본 켜짐)**: 성부 라벨 확정 후 **PDF·MusicXML(OSMD) 나란히 미리보기**와 lint로 Audiveris MXL을 대조한 뒤 **앱 안 MXL 보정**(P 제거, 쉼표 줄·스태프 등) → **보정 MXL에 적용** → **이어하기**로 최종 MXL까지 반영합니다(MuseScore 불필요). [docs/일반_품질_및_HITL_로드맵.md](docs/일반_품질_및_HITL_로드맵.md).
