...
- **Audiveris 직후 OMR 품질 검토(HITL, 기본 켜짐)**: 성부 라벨 확정 후 **PDF·MusicXML(OSMD) 나란히 미리보기**로 대조하고, **오른쪽 악보에서 마디를 클릭**(오선·음표 영역, 호버 시 파란 표시)해 점(·)·쉼표·음표 등을 요소별 보정 → **MXL에 반영·미리보기**로 OSMD에서 확인 → **이어하기**로 최종 MXL까지 반영합니다(MuseScore 불필요). 인쇄 마디 번호 입력은 보조. Audiveris가 점을 `<dot>` 없이 duration에만 반영해 내보낸 쉼표(미리보기에 "없던 점")는 **변환 직후 자동 정규화**되며, 검토 패널의 **「쉼표 길이 자동 정리 (전체 성부)」** 버튼으로도 한 번에 정리할 수 있습니다. [docs/일반_품질_및_HITL_로드맵.md](docs/일반_품질_및_HITL_로드맵.md), [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md) §OMR 품질 검토.
...
| `POST /api/omr-hitl/:jobId/normalize-rests` | 전체 성부 쉼표 duration 자동 정리 (점이 duration에만 반영된 OMR 오류) |
