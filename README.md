...
- **Audiveris 후처리 (최근)**: 세잇단 **쉼표 없음→`show-bracket=no`(숫자만)**, **쉼표 포함→bracket 유지**. **PL 미리보기 clef** — grand staff 줄바꿈 마디에 PR G clef만 있을 때 PL에 G가 씌워지던 OSMD split 버그 수정(저장 MXL 불변). **HITL verbatim split** — PR·PL part마다 `<staves>1</staves>`·clef·note staff 정규화(7줄·PR/PL 라벨 중복·PL 음높이 어긋남 방지, 저장 MXL 불변). **HITL 전에는 `fix_audiveris_mxl` 미적용**(raw MXL 유지). OSMD 미리보기만 m1 조표 생략 시 C major 명시. **조바꿈 clef/조표/pitch 자동 보정은 제거**(HITL·사람 보정). 화음 멤버 beam 일괄 제거 등은 **제거**(범용 오류 유발).
