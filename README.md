...
- **Audiveris 후처리 (최근)**: 세잇단 **쉼표 없음→`show-bracket=no`(숫자만)**, **쉼표 포함→bracket 유지**. **PL 미리보기 clef** — grand staff 줄바꿈 마디에 PR G clef만 있을 때 PL에 G가 씌워지던 OSMD split 버그 수정(저장 MXL 불변). **조바꿈 clef/조표/pitch 자동 보정은 제거**(원본 왜곡 방지). **HITL 미리보기는 Audiveris MXL verbatim**(조표·clef 자동 주입 없음, `audiveris_raw.mxl` 우선). 화음 멤버 beam 일괄 제거 등은 **제거**(범용 오류 유발).
