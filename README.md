...
- **OMR 리듬·화음 slur 보정**: `fix_audiveris_mxl.py` — 세잇단 run 직전 4분 화음은 **duration 중립**이면 total==expected 일 때도 3slice로 펼침. **♩–♪–♩+♪♪** 패턴(끝 4분 유실) 복원. 화음 slur는 **E4 `below`·G4 `above`** 등 성부별 placement(`orientation`/`default-y` 제거). 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
