...
- **OMR 리듬·tie 자동 보정 확대**: `fix_audiveris_mxl.py` 범용 보정 — … ⑩ **PL orphan plain 빔 8분 3개→1절 세잇단**, **빔 run 뒤 4분 화음→8분 2개**, **♩. 보정 후 voice default-x 재정렬**, ⑪ **음표만으로 마디가 맞을 때 overfull 4분→8분 생략**·**♪♪–♩–♪♪ 정상 패턴 4분 분할 생략**, ⑫ **pickup 𝄽8→♪**·**병렬 voice 앞 ♩♩→♪♪**(인쇄 25=XML m24). **임시표 역전파 비활성**. 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
