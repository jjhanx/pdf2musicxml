...
- **한글 가사 인식 검증 (Human-in-the-loop)**: PaddleOCR 결과 중 신뢰도가 낮은 글자가 발견되면 서버가 임시 대기 상태로 전환되며 웹 UI에서 **"글자 인식 확인"** 창이 나타납니다. 사용자가 원본 악보 이미지와 인식 결과를 대조하고 교정하여 변환 품질을 극대화할 수 있습니다.
