...
- **OMR 리듬·tie 자동 보정 확대**: `fix_audiveris_mxl.py` 범용 보정 — ① 세잇단 **`show-number="actual"`**·**stem 방향 기준 placement**(down→below, up→above), ② tuplet 표기 자동 주입, ③ **backup/forward voice 병합**(forward>0·fermata voice는 제외), ④ **4분 2개+세잇단·staccato 8분 3개·𝄽8+staccato/p 8분 2개** 세잇단 복원, ⑤ **마디가 8분 하나 넘칠 때만** ♩. ♩ ♩ 𝄽 / ♩. ♩(voice 2개) 점4분·8분 오인, ⑥ **마디가 4분 하나 넘칠 때만** 4분 2개+8분 run→8분 2개, ⑦ 화음 **중복 pitch·#→natural 오인** 복원, ⑧ **같은 음고 이어짐 slur**·tie·dynamics `p` 오인 등. **음표 발명**만 `omr_score_patches.py`(pitch 시그니처·마디 overfull 조건). 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
