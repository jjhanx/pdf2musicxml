...
- **OSMD 미리보기 박자 간격**: Softmax가 마디 최소폭에서 음표마다 비슷한 간격을 주던 문제를, render 후 **onset 비례 `data-osmd-layout-x` → SVG** 전곡 재배치(양 오선 shared span)로 보정합니다. 저장 MXL·`measure@width` 불변. SoftmaxFactor≥100 보조. 회귀: `npx tsx _smoke/test_onset_layout_beat_spacing.ts`.
