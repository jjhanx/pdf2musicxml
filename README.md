...
- **Audiveris 직후 OMR 품질 검토(HITL, 기본 켜짐)**: 성부 라벨 확정 후 **PDF·MusicXML(OSMD) 나란히 미리보기**로 대조하고, **오른쪽 악보에서 마디를 클릭**(오선·음표 영역, 호버 시 파란 표시)해 점(·)·쉼표·음표·**세잇단**·**빔**·**마디 템포(BPM)** 등을 요소별 보정 → **MXL에 반영·미리보기**로 OSMD에서 확인 → **이어하기**로 최종 MXL까지 반영합니다(MuseScore 불필요). **템포 추가 후 앞에 빈 마디가 생기면** MusicXML에서 tempo `<direction>`이 `<attributes>` 앞에 있는 OSMD 버그 패턴입니다 — HITL·inject·미리보기 정규화로 `attributes` 뒤에 두도록 수정됨([docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md) 「OSMD 미리보기 빈 마디」). **작업 저장(ZIP)/불러오기**로 검토를 중단·재개할 수 있으며(ZIP에 **clean_score·원본 PDF·lyric_manifest.json** 포함), **3단계**에서 **omr-work.zip**으로 Audiveris 없이 이어가거나, **4단계**에서 교정 완료 ZIP + 가사 JSON으로 가사 검증·주입만 진행할 수 있습니다. [docs/일반_품질_및_HITL_로드맵.md](docs/일반_품질_및_HITL_로드맵.md), [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md) §OMR 품질 검토.
