...
- **Audiveris 후처리 핵심 휴리스틱 추가 적용**: 
  - **세잇단 렌더링 정상화 (조건부 괄호 표시)**: OSMD 누락 방지를 위해 세잇단 표기에 `bracket` 속성을 주입하되, 쉼표가 섞여있지 않은 일반적인 세잇단음은 대괄호 선 없이 숫자 '3'만 깔끔하게 표시하고(렌더러의 숫자 소실 및 화음 뭉개짐 방지를 위해 `show-bracket` 속성은 생략), 쉼표가 포함된 특별한 세잇단음의 경우에만 대괄호(bracket)를 보여주도록 보정을 지능화.
  - **점4분음표 뒤 8분음표 오인 보정 강화**: 마디 길이가 초과될 때 점4분음표 직후의 4분음표를 8분음표로 수정할 뿐만 아니라, 점4분음표 뒤에 4분음표 2개와 8분음표 2개가 오며 8분음표 하나만큼 초과되는 경우 두 4분음표를 모두 8분음표로 반감하고 마디 끝에 8분쉼표를 보충하는 정규 리듬 복원 패턴(♪♪♪♪ 리듬 구조 복원) 추가.
