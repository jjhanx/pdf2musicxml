...
- **디버그 모드**: UI에서 "업로드 원본 PDF + Audiveris 결과물 ZIP으로 받기 (디버그)"를 체크하면 **업로드 원본 PDF**와 Audiveris가 만든 **MXL/MusicXML**을 ZIP으로 받습니다.
...
`requirements.txt`는 과거 Python 보조 스크립트용 메모만 남겨 두었습니다. 현재 서버는 **Python 없이** 동작합니다.
