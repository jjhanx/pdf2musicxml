...
- **성부 라벨(S/A/T/B/PR/PL)**: 문자 검토에서 적은 값은 `part_labels_preset.json`(초안)에 저장됩니다. Audiveris 직후 모달에서 **확정**하면 `part_labels.json`이 생깁니다. 확정 없이 초안만 있어도 **최종 MXL·lint**에 초안이 사용됩니다(`PR`·`PL` → **Piano**). PDF **페이지(p.)** 와 구분됩니다.
