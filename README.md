...
- **OMR 리듬·화음 slur 가드 강화**: `fix_audiveris_mxl.py` — 세잇단 run 직전 4분 화음 펼침·plain 8분 3개 세잇단화는 **voice duration 수학**으로만 실행. **다른 staff underfull**이어도 overfull voice는 보정. 화음 slur는 **음머리 쪽 placement + 위 성부 default-y**. 마디 **맨 앞** 연속 4분 2개→빔 8분 쌍 복원. 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
