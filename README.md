...
- **성부 라벨(S/A/T/B/P/PR/PL)**: 문자 검토에서 적은 값은 `part_labels_preset.json`(초안)에 저장됩니다. OMR 직후 **성부 라벨 지정** 모달은 MXL 파트 수와 **문자 검토 preset 중 큰 쪽**(homr가 3파트만 만들어도 SATB+피아노 6성부 표시)을 보여 주며, **파트 수·+ 추가**로 늘릴 수 있습니다. **확정**하면 `part_labels.json`이 생깁니다.
