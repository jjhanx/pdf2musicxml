...
- **비동기 변환(폴링)**: Nginx·Cloudflare 등 앞단 프록시의 **게이트웨이 타임아웃(예: 504)** 을 피하기 위해, 업로드 직후 **HTTP 202** 로 작업 ID만 받고 변환은 서버 백그라운드에서 진행합니다. 클라이언트는 **상태 API를 주기적으로 조회**한 뒤, 완료 시 **다운로드 API**로 결과를 받습니다.
...
| `POST /api/convert` | `multipart/form-data`: 필드 `pdf`(파일), `debug`(`true`/`false`, 선택). 성공 시 **202 Accepted**, 본문 `{ "jobId": "…", "message": "…" }`. 업로드·검증 실패 시 4xx/503 |
...
- **504 Gateway Time-out (역프록시 뒤에서 변환 중 끊김)**: 구버전과 달리 최신 코드는 **장시간 변환을 단일 HTTP 응답으로 연결하지 않습니다.** `/api/convert`는 빠르게 202로 끝나므로 프록시 읽기 타임아웃에 걸리지 않아야 합니다. 여전히 504가 나오면 업로드 크기 제한·`client_max_body_size` 등 **다른 타임아웃/용량 설정**을 점검하세요.
...
- **다운로드된 ZIP 파일 이름이 `ë__Â...` 같은 외계어로 깨지는 현상**: 파일 업로드 시 `multer` 모듈이 한글 파일명을 기본적으로 `Latin-1`으로 잘못 디코딩하여 발생한 문제입니다. 최신 코드에서는 서버가 이를 받아 다시 `UTF-8`로 올바르게 변환하도록 수정되었습니다.
