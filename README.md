...
- **OMR 리듬·화음 slur 보정**: 인쇄 마디 ≈ **XML + 1**(pickup). **인쇄 45 PR = XML m44 staff1** — ♩♩–♪♪–♩↔♪♪–♩♩–♩ 스왑. 화음 slur — **E4+G4** 연속 8분 화음만 **E4 `below`·G4 `above`**. OSMD 미리보기는 stem 쪽 이음줄 비활성. **인쇄 45 PL = XML m44 staff2** — 순차 4분 2개(다른 화음) → **T(A,B,B) 병합**·plain 빔 8분 3개+세잇단 run → 첫 `3` 표기. **OMR 품질 검토** MusicXML 미리보기(`GET …/score-musicxml`)는 요청마다 `fix_audiveris_mxl` 재적용. [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
- **OMR 리듬·tie 자동 보정 확대**: `fix_audiveris_mxl.py` 범용 보정 — … ⑨ **세잇단+일반 8분 혼재 voice** overfull 세잇단화 생략, ⑩ **PL orphan plain 빔 8분 3개→1절 세잇단**, **빔 run 뒤 4분 화음→8분 2개**, **♩. 보정 후 voice default-x 재정렬**, ⑪ **음표만으로 마디가 맞을 때 overfull 4분→8분 생략**·**♪♪–♩–♪♪ 정상 패턴 4분 분할 생략**, ⑫ **𝄽8+♩(오인)→𝄽8+♪**·**병렬 voice 앞 ♩♩→♪♪**(인쇄 25=XML m24), ⑬ **`_measure_rhythm_repairable` 게이트 강화** — voice total==expected만으로 aggressive 보정 진입 금지(m19 등 OMR 유지). **임시표 역전파 비활성**. 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
