...
- **OMR 리듬·tie 자동 보정 확대**: `fix_audiveris_mxl.py` 범용 보정 — ① 세잇단 **`show-number="actual"`**·**stem 방향 기준 placement**, ② tuplet 표기 자동 주입, ③ **4분 2개+세잇단·staccato 8분 3개·𝄽8+staccato/p 8분 2개** 세잇단 복원, ④ **마디 8분/4분 하나 넘침** 시 점4분·4분→8분 오인(♩. ♩ ♩ 𝄽·♩.+backup·빔 run 뒤 4분 등), ⑤ 화음 **중복 pitch**·tie·dynamics `p` 오인 등. **음표 발명**만 `omr_score_patches.py`(pitch 시그니처). 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
- **Audiveris 후처리 핵심 휴리스틱 추가 적용**: 
  - **세잇단 렌더링 및 대괄호(bracket) 제거 로직 개선**: OSMD 렌더러가 `bracket="no"` 속성을 해석할 때 잇단음 구조 자체를 무시하여 수평 좌표가 겹쳐진 채 "하나의 4분 화음"으로 합쳐 그리는 버그 및 숫자 '3'을 소실하는 문제를 방지하기 위해, 쉼표가 없는 잇단음의 경우 `bracket`과 `show-bracket` 속성 자체를 XML에서 완전히 제거하여 숫자 '3'이 정상 기보되고 악보가 겹쳐지지 않도록 해결했습니다.
  - **점4분음표 뒤 리듬 오인식 보정(Pattern G) 범용화**: divisions가 6 또는 12일 때, 마디 내 성부의 duration 시퀀스가 `[dotted_quarter, quarter, quarter, eighth, eighth]`인 오인식 패턴을 감지하여 2번째 음표만 8분음표로 반감하는 `[dotted_quarter, eighth, quarter, eighth, eighth]` 리듬 복원 휴리스틱을 일반화했습니다. 이로 인해 끝에 억지 쉼표가 붙거나 다른 4분음표가 축소되는 부작용을 해소했습니다.
  - **50마디(사용자 기준 51마디) 임시표 오인식 복원 패치**: B major 화음(# 2개)이 제자리표(natural) 2개로 오역되어 첫 화음에 제자리표가 붙고 두 번째 화음의 제자리표가 소실되는 문제를 방지하기 위해, `omr_score_patches.py`에 시그니처 기반 패치(`_patch_piano_m50_accidentals`)를 추가하여 첫 화음의 `D5`에 sharp 임시표를 복원하고, `F5`는 조표에 의해 자동으로 #으로 연주되므로 불필요한 `#` 임시표가 중복 렌더링되지 않도록 accidental 태그를 제거했습니다.
  - **화음(Chord) 내 잘못된 Beam 제거**: 화음의 모든 멤버에 `<beam>`이 붙어 화음이 겹쳐진 블록처럼 렌더링되는 문제를 막기 위해, 화음 소속 음표의 빔 태그를 전역적으로 일괄 제거.
  - **누락된 임시표 역전파(Back-propagation)**: 마디 내 뒤쪽에 명시된 임시표가 있을 경우, 동일 마디 앞쪽에 임시표를 놓친 동일 피치 음표로 임시표(`#`, `b` 등)를 역전파하여 복원 (제자리표 `natural`은 연주 흐름을 깨뜨리지 않도록 역전파 대상에서 안전하게 배제).
  - **잘못 쪼개진 다중 성부(Voice Fragmentation) 병합**: 보표 내 여러 성부의 길이 합이 마디 길이 한도를 넘지 않거나 조각나 있을 때, 이들을 단일 성부로 직렬화(Flatten)하고 X 좌표 기준으로 정렬해 음표가 수평으로 겹치는(Overlap) 현상을 원천 해소.
...
- **성부 라벨(S/A/T/B/P/PR/PL)**: 문자 검토에서 적은 값은 `part_labels_preset.json`(초안)에 저장됩니다. Audiveris 직후 모달에서 **확정**하면 `part_labels.json`이 생깁니다. 확정 없이 초안만 있어도 **최종 MXL·lint**에 초안이 사용됩니다. 단일 **Piano** 파트는 기본 **`P`**, 양손은 **`PR`**·**`PL`**(MusicXML **Piano** / Pno.). PDF **페이지(p.)** 와 구분됩니다.
- **Audiveris 직후 OMR 품질 검토(HITL, 기본 켜짐)**: 성부 라벨 확정 후 **PDF·MusicXML(OSMD) 나란히 미리보기**로 대조하고, **오른쪽 악보에서 마디를 클릭**(오선·음표 영역, 호버 시 파란 표시)해 점(·)·쉼표·음표 등을 요소별 보정 → **MXL에 반영·미리보기**로 OSMD에서 확인 → **이어하기**로 최종 MXL까지 반영합니다(MuseScore 불필요). **작업 저장(ZIP)/불러오기**로 검토를 중단·재개할 수 있습니다. 마디 편집 **길이** 메뉴에 **점4분음표** 등 점 붙은 길이 선택 지원. [docs/일반_품질_및_HITL_로드맵.md](docs/일반_품질_및_HITL_로드맵.md), [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md) §OMR 품질 검토.
