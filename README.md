...
- **Audiveris 직후 OMR 품질 검토(HITL, 기본 켜짐)**: 성부 라벨 확정 후 **PDF·MusicXML(OSMD) 나란히 미리보기**로 대조하고, **인쇄 마디 번호**로 마디를 열어 점(·)·쉼표·음표 등을 요소별 보정 → **MXL에 반영·미리보기**로 OSMD에서 확인 → **이어하기**로 최종 MXL까지 반영합니다(MuseScore 불필요). [docs/일반_품질_및_HITL_로드맵.md](docs/일반_품질_및_HITL_로드맵.md).
