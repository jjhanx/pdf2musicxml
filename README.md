...
- **OMR 리듬·화음 slur 보정**: `fix_audiveris_mxl.py` — 세잇단 run 앞 **서로 다른 4분 화음 2개**는 A,B,B 세잇단으로 합침. 화음 slur는 **전 성부 `below`(stem up) + 위 성부 `default-y`**. **♩–♪–♩+♪♪** 끝 4분 유실 복원. 인쇄 마디 ≈ XML + 1(pickup). 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
