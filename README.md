...
- **가사 자동 공백 토큰화 개선**: 
  - 기존에는 리뷰 UI에서 타입을 '가사(lyrics)'로 수동 변경할 때만 자동 공백(`autoTokenizeLyricsText`)이 적용되었으나, 이제는 PDF에서 처음부터 'lyrics'로 인식되어 로드되는 항목들에 대해서도 초기 로딩 시 자동으로 공백 토큰화가 적용되도록 `normalizeReviewItemsForUi` 로직을 수정했습니다. 이로 인해 음표 간격이 좁아 PyMuPDF가 공백 없이 추출한 가사(예: 여러 파트 중 P4 등)도 자동으로 토큰화됩니다.
  - 가사의 길이가 짧은 경우(예: '산' 등 1~2글자)에도 한글 음절을 정상적으로 인식하여 토큰화할 수 있도록 최소 한글 글자 수 조건을 완화(최소 3글자 → 1글자)했습니다.
...
| `POST /api/review/:jobId/reset-lyrics-initial` | OMR·HITL **이후** `review_needed` 단계만. 원본 PDF **1차 추출**(전체 텍스트·역할 `unknown` 미분류)로 되돌림 |
...
| `GET /api/review/:jobId/lyric-source-info` | `{ hasSavedLyricReview, hasBaseline }` — ZIP 저장 가사 이어하기 가능 여부 |
