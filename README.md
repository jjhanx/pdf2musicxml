...
- **초기 로드 시 가사 자동 분류 해제**: 사용자의 요청에 따라, PyMuPDF가 폰트 크기 휴리스틱을 사용해 추출한 텍스트를 미리 'lyrics(가사)'로 분류하던 동작을 UI 초기 로딩 시 무시하도록(`unknown`으로 표시) 수정했습니다. 이제 추출된 날 것 그대로의 텍스트가 표시되며, 사용자가 HITL(Human-In-The-Loop) 과정에서 직접 타입을 '가사'로 지정하여 문제를 해결할 수 있습니다.
- **짧은 가사 자동 공백 토큰화 버그 수정**: 가사의 길이가 짧은 경우(예: '산' 등 1~2글자)에도 한글 음절을 정상적으로 인식하여 토큰화할 수 있도록 최소 한글 글자 수 조건을 완화(최소 3글자 → 1글자)했습니다.
