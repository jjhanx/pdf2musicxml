...
- **Audiveris 후처리 핵심 휴리스틱 추가 적용**: 
  - **세잇단 렌더링 정상화 (조건부 괄호 표시)**: OSMD 누락 방지를 위해 세잇단 표기에 `bracket` 속성을 주입하되, 쉼표가 섞여있지 않은 일반적인 세잇단음은 대괄호 선 없이 숫자 '3'만 깔끔하게 표시하고, 쉼표가 포함된 특별한 세잇단음의 경우에만 대괄호(bracket)를 보여주도록 보정을 지능화.
  - **점4분음표 뒤 8분음표 오인 보정 강화**: 마디 길이가 초과될 때, 점4분음표 직후의 4분음표를 8분음표로 수정하도록 Scoring 우선순위 상향.
  - **화음(Chord) 내 잘못된 Beam 제거**: 화음의 모든 멤버에 `<beam>`이 붙어 화음이 겹쳐진 블록처럼 렌더링되는 문제를 막기 위해, 화음 소속 음표의 빔 태그를 전역적으로 일괄 제거.
  - **누락된 임시표 역전파(Back-propagation)**: 마디 내 뒤쪽에 명시된 임시표가 있을 경우, 동일 마디 앞쪽에 임시표를 놓친 동일 피치 음표로 임시표(`#`, `b` 등)를 역전파하여 복원.
