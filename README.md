...
- **OMR 리듬·tie 자동 보정 확대**: `fix_audiveris_mxl.py`가 ① 세잇단 잔여 텍스트(`.`/`:2`/`3:2` 등) 제거와 **`show-number="actual"`** 통일 + 숫자 placement를 **빔 쪽(stem 방향)** 으로, ② tuplet 표기 없이 `time-modification`만 있는 잇단 묶음에 **tuplet start/stop 자동 주입**(렌더러가 '3'을 그리도록), ③ 피아노(2단) 잇단 마디에서 '3'이 **dynamics `p`로 오인**된 direction 제거, ④ 마디가 8분음표 하나만큼 넘칠 때 **4분→8분 오인 복구**(점음표 직후·8분쉼표 인접 후보만 보수적으로 — **끝 8분쉼표를 음표로 바꾸지 않음**), ⑤ 빔·잇단 음표의 **빔 쪽 staccato(잇단 '3' 오인) 제거**, ⑥ 화음 부분 tie 완성·**줄바꿈 경계 tie 복원**, ⑦ 악보별 패턴 패치(`scripts/omr_score_patches.py`, 마디 내용 시그니처가 일치할 때만)를 수행합니다. 미리보기(OSMD)는 기본 인쇄 관례상 연속 잇단 숫자를 생략하므로 **항상 표시**하도록 설정했습니다(review.mxl과 미리보기가 달라 보이던 문제). 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
