...
- **OMR 리듬·tie 자동 보정 확대**: `fix_audiveris_mxl.py` 범용 보정 — ① 세잇단 잔여 텍스트·**`show-number="actual"`**·피아노 staff≥2 **`placement=above`**, ② tuplet 표기 자동 주입, ③ **backup/forward voice 병합**, ④ **𝄽+8분 2개·4분 2개+세잇단·staccato 8분 3개** 세잇단 복원, ⑤ **♩. ♪ ♩ 𝄽** / **♩. ♪ ♩.** 점4분·8분 오인, ⑥ **4분 2개+8분 run**·마디 초과 4분→8분, ⑦ 화음 **중복 pitch 제거**·**#→natural 오인** 복원, ⑧ tie·dynamics `p` 오인 등. **음표 발명·성부 재배치**만 `omr_score_patches.py`(pitch 시그니처 일치 시). 자세히 [docs/악보_변환_품질_가이드.md](docs/악보_변환_품질_가이드.md).
