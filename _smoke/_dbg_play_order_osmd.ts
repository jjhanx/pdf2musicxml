...
Object.assign(globalThis, { document: dom.window.document, window: dom.window, DOMParser: dom.window.DOMParser, Node: dom.window.Node, Element: dom.window.Element });
...
async function main() {
  ...
  fs.writeFileSync('_smoke/_tmp_play_order_preview.xml', preview);
  ...
  console.log('notes in xml', preview.match(/<note/g)?.length);
  console.log('stavenotes', host.querySelectorAll('.vf-stavenote,.vf-staveNote').length);
  console.log('host html len', host.innerHTML.length);
}
