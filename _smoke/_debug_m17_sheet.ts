/** Inspect OSMD Sheet after load for m17 voices/timestamps */
...
function buildSlice(raw: string): string {
  ...
  const div = m17.querySelector('attributes divisions,*|attributes *|divisions')?.textContent;
...
async function main() {
  ...
  const sms = (sheet?.SourceMeasures ?? []) as Record<string, unknown>[];
  console.log('source measures', sms.length);
  for (const sm of sms) {
    console.log('mn', sm.MeasureNumber ?? sm.measureNumber, 'dur', (sm.Duration as Record<string, unknown>)?.RealValue);
    for (const vc of (sm.VerticalSourceStaffEntryContainers ?? []) as Record<string, unknown>[]) {
      const ts = (vc.Timestamp ?? vc.timestamp) as Record<string, unknown> | undefined;
      const t = ts?.RealValue ?? ts?.realValue;
      for (const se of (vc.StaffEntries ?? vc.staffEntries ?? []) as Record<string, unknown>[]) {
        if (!se) continue;
        for (const ve of (se.VoiceEntries ?? se.voiceEntries ?? []) as Record<string, unknown>[]) {
          const notes = (ve.Notes ?? ve.notes ?? []) as Record<string, unknown>[];
          const ps = notes.map(pitch).join('+');
          const parent = ve.ParentVoice ?? ve.parentVoice ?? ve.voiceId ?? ve.Voice;
          console.log(`  t=${t} voice=${parent} ${ps}`);
        }
      }
    }
  }
      ...
      for (const se of (vc.StaffEntries ?? []) as Record<string, unknown>[]) {
        ...
        for (const ve of (se.VoiceEntries ?? []) as Record<string, unknown>[]) {
          for (const n of (ve.Notes ?? []) as Record<string, unknown>[]) {
            try {
              const gn = rules.GNote(n) as Record<string, unknown>;
              ...
              console.log(`  t=${t} ${pitch(n)} absX=${abs?.x}`);
            } catch (e) {
              console.log('  err', pitch(n), e);
            }
          }
        }
      }
