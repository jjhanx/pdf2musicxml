/** Inspect OSMD SVG transforms for m17 parallel notes */
...
function buildPr(raw: string): string {
  ...
  return `<?xml version="1.0" encoding="UTF-8"?><score-partwise version="3.1"><part-list><score-part id="P5"><part-name/></score-part></part-list><part id="P5">${partXml}</part></score-partwise>`;
}
...
async function dumpSvgX(osmd: unknown, label: string) {
  console.log(`\n=== ${label} ===`);
    ...
    for (const vc of (sm.VerticalSourceStaffEntryContainers ?? []) as Record<string, unknown>[]) {
      ...
      for (const se of (vc.StaffEntries ?? []) as Record<string, unknown>[]) {
        ...
        for (const ve of (se.VoiceEntries ?? []) as Record<string, unknown>[]) {
          ...
          const voice = ve.voiceId ?? parent?.VoiceId ?? parent?.voiceId;
          for (const n of (ve.Notes ?? []) as Record<string, unknown>[]) {
            ...
            if (svg) {
              ...
              const htmlSvg = svg as unknown as { getBBox?: () => DOMRect };
              if (htmlSvg.getBBox) {
                try {
                  const bb = htmlSvg.getBBox();
                  svgX = String(bb.x + bb.width / 2);
                } catch { /* */ }
              }
            }
            console.log(`v=${voice} ${pitch(n)} absX=${abs?.x} svgX=${svgX} tr=${transform.slice(0, 60)}`);
          }
        }
      }
    }
...
async function main() {
  ...
  await dumpSvgX(osmd, 'before render');
  (osmd as { render: () => void }).render();
  await dumpSvgX(osmd, 'after render before align');
  alignLinkedParallelOnsetGraphics(osmd as never, hints);
  await dumpSvgX(osmd, 'after align (post-render)');
  fs.writeFileSync('_smoke/_m17_svg.html', host.innerHTML);
}
