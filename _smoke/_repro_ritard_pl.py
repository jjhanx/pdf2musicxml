"""Repro: addNoteDirection ritard on PL then check XML; also export for TS preview."""
...
from omr_hitl_lib import apply_fix, _local, _ns  # noqa: E402
...
print("PL apply", ok_pl)
p5 = root.find(".//{*}part[@id='P5']/{*}measure")
for c in p5:
    tag = _local(c.tag)
    if tag == "direction":
        print("DIR", ET.tostring(c, encoding="unicode")[:220])
    elif tag == "note":
        st = c.find("{*}staff")
        print("NOTE staff", st.text if st is not None else "?")
    else:
        print(tag)
