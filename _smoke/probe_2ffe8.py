...
def slurs(n, ns):
    ...
    for s in n.findall(".//" + q(ns, "slur")):
        plc = s.get("placement")
        out.append(f"{s.get('number')}:{s.get('type')} plc={plc if plc else '-'}")
    return out
...
targets = {"6", "28", "30", "41", "42", "43", "44", "45", "46"}
for part in root.findall(q(ns, "part")):
    ...
    for measure in part.findall(q(ns, "measure")):
        ...
        for n in measure.findall(q(ns, "note")):
            ...
            if st == "2" and mn not in {"28", "41", "42", "43", "44", "45", "46"}:
                continue
            if st == "1" and mn in {"28"}:
                continue
            ...
            if n.find(q(ns, "chord")) is None:
                print(f" s{st} {ch}{pitch(n, ns)}:{typ}{tm}({dur}){tup}{extra}")
