...
function countNotesInMeasure(osmd: import('opensheetmusicdisplay').OpenSheetMusicDisplay, partId: string, mn: number): number {
  let n = 0;
...
async function main() {
  ...
  for (const pid of parts) {
    const n26 = countNotesInMeasure(osmd, pid, 26);
    const n27 = countNotesInMeasure(osmd, pid, 27);
    console.log(pid, { m26: n26, m27: n27 });
    if (n26 === 0) throw new Error(`${pid} m26 empty`);
    if (n27 === 0) throw new Error(`${pid} m27 empty — measure swallowed`);
  }

  const p1m26 = countNotesInMeasure(osmd, 'P1', 26);
