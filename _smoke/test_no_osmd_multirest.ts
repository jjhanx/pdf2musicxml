...
async function main() {
  ...
  const defaultReduced = reducedCount(defaultOsmd);
  console.log('default OSMD collapsed rest measures:', defaultReduced);
  ...
  if (n !== 4) {
    throw new Error(`expected 4 source measures, got ${n}`);
  }
