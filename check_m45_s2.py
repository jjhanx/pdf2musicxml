...
for el in meas:
    if el.tag in ('note', 'backup', 'forward'):
        st = el.find('staff').text if el.find('staff') is not None else '1'
        if st == '2':
            if el.tag == 'note':
                p = el.find('pitch/step').text if el.find('pitch') is not None else 'R'
                dur = el.find('duration').text if el.find('duration') is not None else '0'
                v = el.find('voice').text if el.find('voice') is not None else '?'
                c = 'C' if el.find('chord') is not None else ' '
                print(f'{el.tag} V{v} {p}{c} dur={dur}')
            else:
                dur = el.find('duration').text if el.find('duration') is not None else '0'
                print(f'{el.tag} dur={dur}')
