# AI OMR 배포 가이드 (실험 — Local PC · Ubuntu Server)

> **기본 OMR은 Audiveris**(`OMR_ENGINE=audiveris`)입니다. 이 문서는 **`OMR_ENGINE=ai` 실험**용입니다.
...
### 1-4. Audiveris(기본) / AI OMR 전환

| 목적 | 설정 |
|------|------|
| **기본 (Audiveris)** | `OMR_ENGINE=audiveris` + `AUDIVERIS_BIN` |
| AI OMR (실험) | `OMR_ENGINE=ai` + `requirements-ai.txt` |
...
| 변수 | 기본 | 설명 |
|------|------|------|
| **`OMR_ENGINE`** | **`audiveris`(기본)** | Audiveris. AI OMR은 `ai` |
...
| 증상 | 조치 |
|------|------|
| `503 AUDIVERIS_BIN` | `OMR_ENGINE=audiveris`(기본)일 때 `AUDIVERIS_BIN` 설정 |
