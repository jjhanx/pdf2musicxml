...
### 1-2. AI OMR만 사용 (Audiveris 없이)

```powershell
$env:OMR_ENGINE = "ai"
$env:AI_OMR_BACKEND = "mock"          # 파이프라인 연결 확인
...
### 1-3. TrOMR (실제 AI 인식, GPU 권장)

```powershell
pip install -r requirements-ai.txt
$env:OMR_ENGINE = "ai"
$env:AI_OMR_BACKEND = "tromr"
...
### 1-5. Audiveris와 AI 전환

| 목적 | 설정 |
|------|------|
| 기존과 동일 | `OMR_ENGINE` 미설정 + `AUDIVERIS_BIN` 설정 |
| AI OMR | `OMR_ENGINE=ai` |
...
export AI_OMR_BACKEND=mock          # 초기: mock으로 파이프라인 검증
...
### 2-2-B. Audiveris + AI 병행

```bash
export AUDIVERIS_BIN=/opt/audiveris/bin/Audiveris
export OMR_ENGINE=audiveris          # 기본
# AI 테스트 시만: export OMR_ENGINE=ai
```
