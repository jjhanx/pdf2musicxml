...
5. **OMR 페이지·성부 품질 검토** 모달 (MuseScore **불필요**):
   ...
   - 패널을 열면 **`GET …/score-musicxml`** 은 **OMR 검토 중** HITL `review.mxl`을 **재후처리 없이** XML만 추출합니다. OSMD 미리보기도 **verbatim**(성부 필터·PR/PL split만, clef/key/pitch·direction 이주 없음). **「MXL에 반영·미리보기」**·**sync-preview**는 **`audiveris_raw.mxl` 백업 → HITL 보정만 재적용**(`fix_audiveris_mxl` **미적용**)합니다. HITL을 아직 적용하지 않았으면 **raw MXL**을 다시 씁니다. ZIP 불러오기도 **`audiveris_raw.mxl` 우선**입니다.
