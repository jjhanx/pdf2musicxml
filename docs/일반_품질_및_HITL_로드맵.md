...
| 이음줄이 음표 위/아래에 잘못 붙거나 빔과 너무 가까움 | HITL 이음줄 **위치 위/아래 + 오선 거리(1-10칸)** (`addSlur`·`setSlurPlacement`). 저장 MXL의 `<slur>` 양 끝에 `default-y`와 `data-hitl-slur-distance`를 기록하고, 미리보기 load XML에도 같은 힌트를 심음. 검증: `python _smoke/test_slur_placement_hitl.py` · `python _smoke/test_slur_distance_hitl.py` · `npx tsx _smoke/test_slur_distance_preview.ts` | 마디 편집. 곡·마디 하드코딩 없음 |
