...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | Accent는 음표에 유지. 거리=`--hitl-art-dy` CSS translate(`.vf-modifiers`). `:not([transform])` 허점 수정. `npx tsx _smoke/test_articulation_css_shift.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
