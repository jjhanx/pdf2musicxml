...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입**. mid clef 보존·trailing 앵커·동일 내용 중복 제거 시 mid 전환 마디 머리 유지. **끝 clef 뒤 음표 삽입** 시 staff rebuild가 clef를 버리지 않음. **온음표만 있는 마디 끝 mid clef**는 preview 앵커가 divisions 세분 후 peel(저장 MXL 순서 불변). **OMR clef 오인** 시 「오선 위치 유지」(`remapStaffPitches`)로 뒤쪽·범위 음높이를 중선 기준으로 일괄 변환(기본=음높이 유지). `npx tsx _smoke/test_mid_clef_keeps_header.ts` · `python _smoke/test_insert_note_after_trailing_clef.py` · `npx tsx _smoke/test_whole_trailing_g_clef_osmd.ts` · `npx tsx _smoke/test_pl_whole_trailing_g_clef.ts` · `python _smoke/test_clef_remap_staff_pitches_hitl.py` | 「여기 뒤」 후 𝄞/𝄢 · clef 「여기 뒤」에 음표 추가. 온음 뒤 G가 앞으로 가지 않음. clef 오인이면 「오선 위치 유지」 |
