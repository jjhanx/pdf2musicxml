...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | Accent는 음표에 유지. 거리는 `.vf-modifiers` SVG `transform` (`applySvgDyToVfModifiers`). OSMD가 덮어쓰면 observer로 재적용. CSS `translate`는 SVG에서 무시됨. `npx tsx _smoke/test_articulation_css_shift.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
