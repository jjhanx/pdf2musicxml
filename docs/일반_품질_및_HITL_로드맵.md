...
| 다성·다른 박자에서 연주순번이 갈라짐 | 숫자 순번은 voice별 진행. **다른 voice 열에 맞출 때만 `5-6`**. timeline 기본으로 해석 + 앵커 **실제 layout onset** + SVG에서 voice 열 timestamp 짝짓기(동일 pitch 5·6열 혼동 방지). `python _smoke/test_play_order_voice_ref.py` · `npx tsx _smoke/test_play_order_voice_ref_layout.ts` · `npx tsx _smoke/test_play_order_ref_same_pitch_anchor.ts` | 순번 칸에 `5-6` 입력 후 MXL 반영 |
