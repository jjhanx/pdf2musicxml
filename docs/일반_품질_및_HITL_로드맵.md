...
5. **OMR 페이지·성부 품질 검토** 모달 (MuseScore **불필요**):
   ...
   - 패널을 열면 **`GET …/score-musicxml`** 은 **OMR 검토 중** `syncOmrReviewMxl`(raw+HITL) 후 XML만 추출합니다(**`fix_audiveris_mxl`·rest 정규화 미적용**). OSMD 미리보기도 **verbatim**: part 라벨·PR/PL split(해당 줄 음표·clef 필터만) 외 **clef/key/voice/direction 변환 없음**. 「OMR 자동 정리」만 예외적으로 후처리 실행.
