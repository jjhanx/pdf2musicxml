...
| 다성·다른 박자에서 연주순번이 갈라짐 | 같은 staff·다른 voice·**같은 연주순번**=동시 column. **명시 setPlayOrder 유지**(onset 자동 통일 없음; sanitize는 같은 pitch 충돌만). 미리보기는 `applyPlayOrderLayout`(SVG 상대 snap 비활성). `python _smoke/test_play_order_same_onset_unify.py` | 맞출 열의 순번을 직접 같게. voice만으로 위치 맞추지 않기 |
