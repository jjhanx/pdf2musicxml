...
| tenuto·accent 거리 미리보기 | **반영 전:** `pendingFixes` → 글리프 Δ. **반영 후:** `artPreviewFixes`. **한 음 복수 표:** 표별 Δ(tenuto·accent 독립). UI: `Tenuto 거리` / `추가할 표 거리`. 검증: `npx tsx _smoke/test_dual_articulation_distance.ts` · `npx tsx _smoke/test_articulation_yshift_draw.ts` | 3칸→Δ=20px → 「MXL에 반영」 후에도 위치·헤더 유지 |
