...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | Accent 미리보기는 **OSMD Direction(words)**. 1칸=오선 근처(OSMD 기본~4칸에서 당김), 큰 칸=더 밖. `npx tsx _smoke/test_articulation_as_direction.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
