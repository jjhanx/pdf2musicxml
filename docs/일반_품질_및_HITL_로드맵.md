...
| 셈여림 점선(wedge) 없음·길이 오류 | HITL **셈여림 점선** 패널에서 시작→끝 추가, `wedge(stop)`을 끝 음 **뒤**로 이동(barline 금지). **PR/PL은 staff별로 독립**. 새 점선: 시작·끝 선택이 부모 리렌더에 덮어쓰이지 않음(기본 끝=현재 마디 마지막). OSMD split: direction staff 유지. `python _smoke/test_ornament_wedge_hitl.py` · `npx tsx _smoke/test_wedge_last_note_preview.ts` · `npx tsx _smoke/test_b02f_pl_wedge_reanchor.ts` | 시작·끝 고른 뒤 「점선 추가」→ 첫 반영부터 그 구간 |
...
| 다성·다른 박자에서 연주순번이 갈라짐 | **partial voice:** layout column → **timeline realign**(faithful, 주) → SVG 보조. PL/PR staff 필터는 `targetStaffMatchesGraphic`. 전역 po cluster snap 금지. **`npx tsx _smoke/test_partial_voice_regression.ts`** (필수) · `python _smoke/test_play_order_voice_ref.py` · `npx tsx _smoke/test_play_order_voice_ref_layout.ts` | 순번 칸에 `5-6` 입력 후 MXL 반영 |
