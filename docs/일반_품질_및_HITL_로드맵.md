...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입** → `insertClef`. mid clef는 직전 음에 고정. 미리보기: courtesy 제거·voice-layer normalize가 mid clef를 머리로 올리지 않음. `npx tsx _smoke/test_mid_clef_survives_voice_normalize.ts` 등 | 「여기 뒤」 후 𝄞/𝄢 · 새로고침 후 미리보기 |
