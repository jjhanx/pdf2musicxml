...
| tenuto·accent 거리 미리보기 | pending 표를 `articulationHintXml`로 load XML에 심어 m.49와 동일 경로. 거리 변경 시 remount. 복수 표는 overlay(절대 Y). 검증: `test_m51_load_xml_has_arts.ts` | 51마디 #1 B4 Tenuto/Accent 거리 독립 |
