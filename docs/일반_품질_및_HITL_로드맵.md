...
| 이미지 PDF OMR 검토에서 페이지 넘김이 멈춤·응답 없음 | `pipelineMode=image_pdf`일 때 HITL OSMD는 **성부+선택 마디 1개**만, PNG DPI 96, 페이지 전환은 transition/deferred. `npx tsx _smoke/test_page_measure_range.ts` | Ctrl+Shift+R 후 「마디 열기」·「이 페이지 첫 마디」 |
