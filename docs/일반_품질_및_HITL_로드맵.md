...
| 다성·다른 박자에서 연주순번이 갈라짐 | 숫자 순번은 voice별 진행. **다른 voice 열에 맞출 때만 `5-6`**. timeline 기본으로 해석 + 앵커 **실제 layout onset**(혼합 박자에서 균등 6/7열→6·7 사이 어긋남 방지) + SVG `effectivePlayOrder`. `python _smoke/test_play_order_voice_ref.py` · `npx tsx _smoke/test_play_order_voice_ref_layout.ts` | 순번 칸에 `5-6` 입력 후 MXL 반영 |
