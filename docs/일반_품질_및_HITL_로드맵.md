...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입**. 미리보기: mid clef 보존 + **같은 레이어 후속 음 없는 clef**는 비가시 쉼표 앵커. **동일 내용** 중복 clef 제거 시 **mid 전환 마디의 머리 clef는 유지/주입**(OSMD가 mid F를 앞으로 끌어올리지 않게). `npx tsx _smoke/test_mid_clef_trailing_osmd_anchor.ts` · `npx tsx _smoke/test_redundant_same_clef_strip.ts` · `npx tsx _smoke/test_mid_clef_keeps_header.ts` | 「여기 뒤」 후 𝄞/𝄢 · Ctrl+Shift+R 후 확인 |
