...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입** → `insertClef`. 목록 표시·삭제·clef 뒤 삽입. rebuild·순번 재배열·OSMD flatten이 **같은 staff**에 mid clef 보존(PL로 넘어가 안 보이던 회귀 수정). `python _smoke/test_insert_clef_mid_measure.py` · `python _smoke/test_insert_clef_same_staff_rebuild.py` · `python _smoke/test_normalize_preserves_mid_clef.py` | 「여기 뒤」 후 𝄞/𝄢 · 목록에서 삭제/그 뒤 삽입 |
