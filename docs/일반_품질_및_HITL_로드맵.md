...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | Accent 거리: VexFlow `y_shift`는 between_lines에 막힘 → `.vf-modifiers` 래퍼 `transform`. `npx tsx _smoke/test_articulation_yshift_draw.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
