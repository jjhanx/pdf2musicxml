...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | 오선 **1칸×숫자칸** MXL + SVG `.vf-modifiers` **칸×오선간격**. 거리 드롭다운은 OSMD 재로드 없이 같은 줄 순회(`forEachGraphicalMeasure`)·피치 매칭으로 즉시 이동. `npx tsx _smoke/test_articulation_pending_preview.ts` | 마디 편집 거리: 보통(1칸)~10칸. 저장 MXL은 「MXL에 반영」 |
