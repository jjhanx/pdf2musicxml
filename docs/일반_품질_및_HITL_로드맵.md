...
| 다성·다른 박자에서 연주순번이 갈라짐 | 숫자 순번은 voice별 진행. **다른 voice 열에 맞출 때만 `5-6`**(voice-순번). layout+참조 SVG. `python _smoke/test_play_order_voice_ref.py` · `npx tsx _smoke/test_play_order_voice_ref_layout.ts` | 순번 칸에 `5-6` 입력 후 MXL 반영 |
