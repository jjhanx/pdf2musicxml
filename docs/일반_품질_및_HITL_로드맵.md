...
| PR 8va가 마디 끝에서 최종 MXL에 안 보임 · 미리보기 `realValue` 크래시 | stop이 backup 뒤로 밀림 → `repair_octave_shift_stops…` + `fix_audiveris`에서도 복구. **미리보기는 OSMD 한계로 octave-shift를 words(8va)로 치환**(엔진 크래시 방지). `python _smoke/test_octave_shift_stop_before_backup.py` | 프론트 재빌드·결과 재생성 |
