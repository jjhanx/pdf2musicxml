...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | 오선 **1칸×tier** MXL + SVG `.vf-modifiers` **staff-space×tier** (`applyOsmdArticulationOffsets`). `npx tsx _smoke/test_articulation_preview_shift.ts` | 마디 편집 거리: 1칸/2칸/3칸 |
