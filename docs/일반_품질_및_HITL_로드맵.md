...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | 거리 드롭다운: **raw MXL에 먼저** `data-hitl-art-distance` 반영 후 미리보기 빌드(PR/PL 분할 뒤 noteIndex 오적용 방지) + SVG 칸×오선간격. `npx tsx _smoke/test_articulation_pending_preview.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
