...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | **mf와 다름:** OSMD는 direction `default-y`만 쓰고 Accent XML `default-y`는 VexFlow가 무시. `Articulation.draw`에 `y_shift`(칸−1)를 더해 다시 그림. `npx tsx _smoke/test_articulation_pending_preview.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
