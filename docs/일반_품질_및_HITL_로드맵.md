...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | Accent 거리: 호스트 `--hitl-art-dy` + CSS `transform:translateY`(SVG는 `translate` 속성 무시). `npx tsx _smoke/test_articulation_css_shift.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
