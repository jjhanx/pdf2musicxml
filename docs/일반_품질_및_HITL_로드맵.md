...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | Accent는 VexFlow를 타지 않고 **OSMD Direction(words)** 로 미리보기(저장 MXL은 articulation 유지). 거리=칸 수는 OSMD Label SVG. `npx tsx _smoke/test_articulation_as_direction.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
