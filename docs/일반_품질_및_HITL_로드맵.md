...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | Accent는 **음표에 유지**(mf Direction과 분리). 거리=그 마디 같은 표 글리프 SVG 이동. `npx tsx _smoke/test_articulation_pending_preview.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
