...
| tenuto·accent 거리 미리보기 | **복수 표:** 네이티브 path를 표별 절대 Y로 이동(`path d`·getYs). UI: `Tenuto 거리` / `추가할 표 거리`. 검증: `npx tsx _smoke/test_dual_articulation_distance.ts` · `npx tsx _smoke/test_dual_art_ui_path.ts` · `npx tsx _smoke/test_dual_art_dom_probe.ts` | 2칸·5칸 → visualY 독립. getBBox(0,0) overlay 금지 |
