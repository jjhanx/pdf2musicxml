...
| 다성·다른 박자에서 연주순번이 갈라짐 | 같은 staff·다른 voice·**같은 연주순번**=동시 column. 같은 musical onset에 1과 6처럼 어긋난 순번은 편집기/반영 시 자동 통일. SVG는 같은 순번 cross-voice 상대 snap(절대 grid 금지). `python _smoke/test_play_order_same_onset_unify.py` | 동시에 칠 음 순번을 같게. voice만으로 위치 맞추지 않기 |
