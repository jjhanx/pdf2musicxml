...
5. **OMR 페이지·성부 품질 검토** 모달 (MuseScore **불필요**):
   ...
   - 패널을 열면 **`GET …/score-musicxml`** 은 **OMR 검토 중** `syncOmrReviewMxl`(raw+HITL) 후 XML만 추출합니다(**`fix_audiveris_mxl`·rest 정규화 미적용**). OMR 직후·ZIP import 시에도 HITL 켜진 job은 **`audiveris_raw.mxl`로 작업 MXL을 되돌립니다**. OSMD 미리보기는 **load 직전에만** m1 C major 명시(저장 MXL 불변). **「MXL에 반영·미리보기」**·**sync-preview**는 HITL 보정만 재적용합니다.
