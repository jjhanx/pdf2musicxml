...
| 이미지 PDF OMR 검토에서 페이지 넘김이 멈춤·응답 없음 | `image_pdf`(또는 `imagePdfOmrEngine`): 페이지↔마디 인덱스 1회 파싱, PNG 72DPI·maxSide 1200·페이지수 캐시, OSMD는 성부+선택 마디 1개, **마디 번호/◀마디/마디▶**로 PDF 페이지 동기. `npx tsx _smoke/test_page_measure_range.ts` | Ctrl+Shift+R 후 「마디로 이동」 |
