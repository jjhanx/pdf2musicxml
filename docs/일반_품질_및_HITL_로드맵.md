...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | Accent는 음표에 유지. OSMD `render()`가 modifier 노드를 교체하므로 거리는 host `data-hitl-art-dy` + render 직후 SVG `style.transform`. `npx tsx _smoke/test_articulation_css_shift.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
