...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입**. 미리보기: mid clef 보존 + **같은 레이어 후속 음 없는 clef**(마디 끝·backup 직전)는 비가시 쉼표 앵커로 OSMD in-staff 표시. **이미 유효한 clef와 동일 내용**인 머리/중간/끝 중복은 `removeRedundantCourtesyClefsForOsmd`로 제거(전환만 유지). `npx tsx _smoke/test_mid_clef_trailing_osmd_anchor.ts` · `npx tsx _smoke/test_3c30_m27_pl_clef.ts` · `npx tsx _smoke/test_redundant_same_clef_strip.ts` | 「여기 뒤」 후 𝄞/𝄢 · Ctrl+Shift+R 후 확인 |
