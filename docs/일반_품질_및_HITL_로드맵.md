...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입** → `insertClef`. 목록 표시·삭제·clef 뒤 삽입. rebuild·순번 재배열·OSMD flatten이 **같은 staff**에 mid clef 보존. **clef 뒤 화음**도 attributes 건너뛰기·화음 그룹 밖 mid attributes 복구·리더 인덱스 예측으로 clef 유지. `python _smoke/test_insert_clef_mid_measure.py` · `python _smoke/test_insert_clef_same_staff_rebuild.py` · `python _smoke/test_normalize_preserves_mid_clef.py` · `python _smoke/test_clef_chord_keeps_clef.py` | 「여기 뒤」 후 𝄞/𝄢 · clef **「여기 뒤」**로 화음 추가 · 목록에서 삭제 |
