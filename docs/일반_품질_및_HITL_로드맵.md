...
| tenuto·accent 거리 미리보기 | 네이티브는 ~1칸 겹침(OSMD가 default-y 무시). HITL은 표별 path 이동. **대기 표에도 거리 UI**, 스택 기본 +2칸, 거리 변경 시 remount 없음. 검증: `npx tsx _smoke/test_art_draw_isolation.ts` · `test_m51_b4_dual_art.ts` | 51마디 #1 B4 Tenuto/Accent 거리 독립 |
