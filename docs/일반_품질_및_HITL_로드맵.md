...
5. **OMR 페이지·성부 품질 검토** 모달 (MuseScore **불필요**):
   ...
   - 패널을 열면 **`GET …/score-musicxml`** 은 **OMR 검토 중** HITL 반영 `review.mxl`을 **재후처리 없이** XML만 추출합니다(빔·박자 등 HITL 보정 유지). **「MXL에 반영·미리보기」**·**sync-preview**는 **`audiveris_raw.mxl` 백업 → `fix_audiveris_mxl`(조표 `<key>`·`<clef>`·pitch **기본 Audiveris 그대로**) → HITL 보정 재적용**으로 MXL 파일을 재합성합니다.
