...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입** → `insertClef`. **낮은음으로 둘 마지막 음**의 「여기 뒤」에 넣고 그 다음 음만 새 clef. backup 직후 mid clef가 마디 머리로 승격되어 앞 마디 예고·전체 적용되던 rebuild 버그 수정. `python _smoke/test_insert_clef_mid_measure.py` · `python _smoke/test_insert_clef_same_staff_rebuild.py` · `python _smoke/test_normalize_preserves_mid_clef.py` · `python _smoke/test_clef_chord_keeps_clef.py` · `python _smoke/test_clef_after_backup_not_header.py` | 「여기 뒤」 후 𝄞/𝄢 · clef 뒤 음표/화음 · 깨진 경우 삭제 후 재삽입 |
