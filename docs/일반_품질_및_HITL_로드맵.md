...
| PR에 원본 없는 온쉼표·PL 온음이 그쪽으로 밀림 | cross-staff <backup> 누락. **단일 voice/staff 마디도** timeline normalize에서 backup 복구(예전엔 다성만 처리). staff2 voice=1→5. python _smoke/test_piano_m1_cross_staff_backup.py · python _smoke/test_bf4e_phantom_rest_backup.py | HITL 동기화·결과 재생성 |
| PL 다성 8분쉼·2분음 위아래가 바뀜 | 쉼표만 있는 onset에서도 성부 stem 정규화(낮은 voice#=up). 짧은 쉼 display는 **같은 voice 실음 쪽** 유지. python _smoke/test_bf4e_phantom_rest_backup.py · python _smoke/test_pin_polyphonic_short_rests.py | OMR 검토 미리보기·MXL 반영 |
