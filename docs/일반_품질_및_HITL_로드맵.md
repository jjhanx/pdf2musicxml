...
| 다성부 짧은 쉼이 다른 voice 음과 겹치거나 오선 밖 | 동시 다른 voice 음의 **반대편·오선 안** `display-step`. `python _smoke/test_pin_polyphonic_short_rests.py` · `npx tsx _smoke/test_rest_display_osmd_preview.ts` | Ctrl+Shift+R / OMR 자동 정리 |
