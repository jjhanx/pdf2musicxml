...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입** → `insertClef`. mid clef는 직전 음에 고정. 미리보기: courtesy·voice-layer·**mid에 staves 주입 금지**로 OSMD in-staff clef 유지. `npx tsx _smoke/test_mid_clef_osmd_after_transform.ts` 등 | 「여기 뒤」 후 𝄞/𝄢 · 강력 새로고침 후 미리보기 |
