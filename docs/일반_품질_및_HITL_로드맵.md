...
| PL 중간 G→F 후 다음 마디가 최종 MXL에서 높은음자리 | `fix_audiveris_mxl`이 staff2 F·중간 전환을 조바꿈 오인/courtesy로 삭제. staff2 F 유지 + 같은 마디 앞 clef 기준 courtesy. `python _smoke/test_pl_mid_clef_f_survives_fix.py` | 결과 재생성·「OMR 자동 정리」 |
