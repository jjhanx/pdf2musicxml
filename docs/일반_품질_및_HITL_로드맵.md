...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입**. mid clef 보존·뒤에만 적용(**앞 음·앞 clef 불변**). **오선 위치 유지** 시 이후 마디의 잔류 old clef(F 등)를 new(G)로 교체+pitch remap(G 뒤 F 잔류·의도 무력화 금지). 온음 끝 mid clef는 preview 앵커(저장 MXL 불변). `python _smoke/test_clef_insert_forward_only_hitl.py` · `npx tsx _smoke/test_pl_end_g_clef_preview_intent.ts` · `npx tsx _smoke/test_whole_trailing_g_clef_osmd.ts` · `npx tsx _smoke/test_pl_whole_trailing_g_clef.ts` | 「여기 뒤」에 𝄞/𝄢. 앞은 그대로·뒤만 새 clef·(선택) remap. 온음 뒤 G가 앞으로 가지 않음 |
