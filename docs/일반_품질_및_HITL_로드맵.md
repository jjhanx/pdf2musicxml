...
5. **OMR 페이지·성부 품질 검토** 모달 (MuseScore **불필요**):
   ...
   - 패널을 열면 **`GET …/score-musicxml`** 은 **OMR 검토 중** `syncOmrReviewMxl`(raw+HITL) 후 XML만 추출합니다(**`fix_audiveris_mxl`·rest 정규화 미적용**). OSMD 미리보기는 **verbatim**: part 라벨·PR/PL split 외 **저장 MXL 변환 없음**. 단, OSMD가 뒤쪽 조바꿈을 첫머리 4♯로 당겨 그리는 것을 막기 위해 **load 직전 미리보기 전용**으로 m1 C major 명시·조바꿈 F clef 오인 완화만 적용합니다. 「OMR 자동 정리」만 예외적으로 후처리 실행.
