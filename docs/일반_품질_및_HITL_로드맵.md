...
2. **원본 PDF** PNG 미리보기에서 가사·제목·템포 등 역할을 확인·수정합니다. 서버는 **`input.pdf`·`original.pdf`·업로드 경로** 순으로 가사 소스를 찾고 **`clean_score_only.pdf`는 최후**에만 씁니다(가사가 strip된 PDF이므로). 업로드 원본은 세션 `input.pdf`로 복사해 ZIP 복원·3~4단계 재개와 경로를 통일합니다. 원본이 없으면 단계를 **생략**하고 작업 표에 `가사 검증 생략 — 원본 PDF…`가 표시됩니다(특정 악보만의 예외가 아님).
