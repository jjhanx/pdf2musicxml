...
5. **OMR 페이지·성부 품질 검토** 모달 (MuseScore **불필요**):
   ...
   - **MusicXML(OSMD) 악보에서 마디 클릭**으로 마디를 열고 direction·쉼표·음표·점(·)·이음줄 등을 요소별로 보정(덧점 없애기, 쉼표 줄 이동, 삭제·추가 등) → `omr_hitl_fixes.json`에 쌓음. 인쇄 마디 번호 입력은 보조 수단. 구현: `src/osmdMeasureClick.ts`가 OSMD 그래픽·**렌더된 SVG DOM** 좌표로 마디별 클릭 영역을 만들고, 스크롤·확대 시 위치를 다시 맞춥니다(`musicSystems`/`graphicalMeasures` 등 camelCase 필드 포함).
