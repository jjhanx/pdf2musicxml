...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입**(여기 뒤) → `insertClef`. 목록에 중간·끝 clef 표시·삭제(`removeClef`)·clef 뒤 삽입(`afterClefIndex`). rebuild가 중간 attributes 보존. `python _smoke/test_insert_clef_mid_measure.py` | 「여기 뒤」 선택 후 𝄞/𝄢 · 목록에서 삭제/그 뒤 삽입 |
