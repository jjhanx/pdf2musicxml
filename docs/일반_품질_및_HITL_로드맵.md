...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입**. mid clef 보존·trailing 앵커·동일 내용 중복 제거 시 mid 전환 마디 머리 유지. **끝 clef 뒤 음표 삽입** 시 staff rebuild가 clef를 버리지 않음. `npx tsx _smoke/test_mid_clef_keeps_header.ts` · `python _smoke/test_insert_note_after_trailing_clef.py` | 「여기 뒤」 후 𝄞/𝄢 · clef 「여기 뒤」에 음표 추가 |
