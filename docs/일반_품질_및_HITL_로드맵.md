...
| 다성·다른 박자에서 연주순번이 갈라짐 | **미리보기:** partial voice layout → timeline realign(faithful, 병행 half trim 금지) → SVG 보조. **저장·최종 MXL 재생:** `realign_play_order_column_timelines_in_root`가 같은 명시 순번·참조 column onset을 맞춤(trim/forward 허용). `npx tsx _smoke/test_partial_voice_regression.ts` · `python _smoke/test_play_order_timeline_realign_persist.py` · `python _smoke/test_play_order_voice_ref.py` | 순번 맞춘 뒤 「MXL에 반영」·결과 생성 |
