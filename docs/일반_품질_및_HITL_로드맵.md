...
| 두 마디에 걸친 diminuendo/crescendo를 못 그림 | 마디 편집 **「다음 마디 포함」** + wedge `toMeasureMxl`(start m.N · stop m.N+1). **이미지 PDF 경량 OSMD**는 `lightOsmdPreviewMeasureRange`로 선택+다음 마디를 함께 그려 hairpin·다음 마디를 눈으로 확인. `python _smoke/test_cross_measure_wedge_hitl.py` · `npx tsx _smoke/test_page_measure_range.ts` | 42마디 열고 포함 켠 뒤 끝 음을 43마디에서 선택 → MXL 반영 후 미리보기에 dim+43마디 |
