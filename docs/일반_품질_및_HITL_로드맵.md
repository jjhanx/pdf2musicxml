...
| PL HITL 빔이 앞쪽·위 오선 쪽으로 깨져 보임 | Audiveris `<stem default-y>`(·beam 좌표)가 note의 default-x/y만 지운 채 남아 OSMD 빔 geometry가 틀어짐. `stripDefaultXy*`가 stem·beam 좌표도 제거하고, `applyBeam`/`_set_note_stem`도 stem 좌표를 지움. 검증: `npx tsx _smoke/test_6497_m10_pl_beam.ts` | OSMD 미리보기·빔 적용. 곡·마디 하드코딩 없음 |
