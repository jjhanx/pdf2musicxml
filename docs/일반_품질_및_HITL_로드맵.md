...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입**. mid clef 보존·뒤에만 적용(**앞 음·앞 clef 불변**). **오선 위치 유지가 기본**. 맨 끝 삽입은 현재 마디 불변·다음 마디부터 머리 clef+pitch remap(첫 음 전 attrs 전부·mid other 앞까지). 다성부 staff 끝은 문서 순서가 아니라 MusicXML onset 기준으로 판단해 `backup` 뒤 병렬 voice를 앞 음으로 보존. `python _smoke/test_clef_insert_forward_only_hitl.py` · `python _smoke/test_insert_clef_multivoice_staff_end.py` · `npx tsx _smoke/test_pl_end_g_clef_preview_intent.ts` | 음높이 모드를 **먼저** 고른 뒤 𝄞/𝄢. 앞은 그대로·뒤·다음 마디 remap. 온음 뒤 G가 앞으로 가지 않음 |
