...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입** → `insertClef`. **낮은음으로 둘 마지막 음**의 「여기 뒤」에 넣고 그 다음 음만 새 clef. mid clef는 직전 음에 고정(다음 음·마디 머리로 자동 이동 금지). `python _smoke/test_clef_stays_after_anchor.py` · `python _smoke/test_clef_after_backup_not_header.py` 등 | 「여기 뒤」 후 𝄞/𝄢 · 깨진 경우 삭제 후 재삽입 |
