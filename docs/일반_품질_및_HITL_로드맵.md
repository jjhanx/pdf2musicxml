...
| 이음줄이 음표 위/아래에 잘못 붙거나 빔과 너무 가까움 | HITL 이음줄 **위치 위/아래 + 오선 거리(1-10칸)** (`addSlur`·`setSlurPlacement`). 저장 MXL의 `<slur>` 양 끝에 `default-y`와 `data-hitl-slur-distance`를 기록하고, PR/PL 필터 미리보기에서는 `noteIndex`가 어긋나도 pitch+slur end fallback으로 대상 slur를 찾는다. OSMD가 `default-y`/render 전 bezier 변경을 최종 SVG에 안정적으로 반영하지 않으므로, render 직후 `.vf-stavetie`/`.vf-curve` path의 절대 Y 좌표를 직접 보정한다. 검증: `python _smoke/test_slur_placement_hitl.py` · `python _smoke/test_slur_distance_hitl.py` · `npx tsx _smoke/test_slur_distance_preview.ts` · `npx tsx _smoke/test_beam_slur_clearance.ts` · `npx tsx _smoke/test_slur_distance_osmd_render.ts` | 마디 편집. 곡·마디 하드코딩 없음 |
| 빔과 이음줄이 같은 쪽에 겹침 | 미리보기: 빔+줄기쪽 placement면 `prepareGraphicalSlursForOsmdPreview`가 기존 자동 1칸 clearance만 유지하고, 사용자가 지정한 거리는 render 후 `applyOsmdSlurDistanceOffsets`가 SVG path에 적용한다. 명시 거리가 없으면 기존 1칸 자동 보정 유지. `npx tsx _smoke/test_beam_slur_clearance.ts` · `npx tsx _smoke/test_slur_distance_osmd_render.ts` | OSMD 미리보기 자동. 곡·마디 하드코딩 없음 |
