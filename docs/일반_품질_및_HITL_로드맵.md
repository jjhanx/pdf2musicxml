...
| 다성·다른 박자에서 연주순번이 갈라짐 | 같은 staff·다른 voice·**같은 연주순번**=동시 column. 같은 musical onset에 1과 6처럼 어긋난 순번은 편집기/반영 시 자동 통일. 미리보기는 `applyPlayOrderLayout`(SVG 상대 snap 비활성 — 빔이 앞 마디로 삐져나오던 회귀). `python _smoke/test_play_order_same_onset_unify.py` | 동시에 칠 음 순번을 같게. voice만으로 위치 맞추지 않기 |
