...
| 마디 중간에 음자리표 전환 필요 | 삽입 폼 **음자리표 삽입**. mid clef 보존·trailing 앵커·동일 내용 중복 제거 시 mid 전환 마디 머리 유지. **끝 clef 뒤 음표 삽입** 시 staff rebuild가 clef를 버리지 않음. **온음표만 있는 마디 끝 mid clef**는 preview 앵커가 divisions 세분 후 peel(저장 MXL 순서 불변). `npx tsx _smoke/test_mid_clef_keeps_header.ts` · `python _smoke/test_insert_note_after_trailing_clef.py` · `npx tsx _smoke/test_whole_trailing_g_clef_osmd.ts` · `npx tsx _smoke/test_pl_whole_trailing_g_clef.ts` | 「여기 뒤」 후 𝄞/𝄢 · clef 「여기 뒤」에 음표 추가. 온음 뒤 G가 앞으로 가지 않음 |
...
| 파트의 앞머리 또는 중간 음자리표가 잘못 배정됨 (예: Alto가 F clef로 고정) | HITL **음자리표 변경** (`setMeasureClef`). **기본=현재 마디만**. 곡 전체는 m.1에만 쓰고 이후 머리 clef를 지움. 마디 단위 OSMD는 앞 마디 상속 clef를 구간에 주입. staff@number 매칭(PR/PL). `python _smoke/test_set_measure_clef_hitl.py` | 마디 편집 「음자리표 변경」→「현재 마디만」→ MXL 반영 |
