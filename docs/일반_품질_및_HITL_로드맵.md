...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | OSMD는 direction `default-y`만 씀. Accent는 VexFlow `default-y` 무시 → **글리프 SVG transform** + 조표 F♯는 `halfTone`(vfpitch=F4 허점). `npx tsx _smoke/test_articulation_pending_preview.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
