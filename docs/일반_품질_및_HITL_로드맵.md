...
2. **원본 PDF** PNG 미리보기에서 가사·제목·템포 등 역할을 확인·수정합니다. **2·4단계**(`clean_score`·`lyric_inject` + `lyric_manifest.json`)로 시작하면 **1단계에서 편집·저장한 manifest 항목**(역할·성부 순번·bbox·수동 마스킹 영역)을 **그대로** 이어 받습니다. `lyric_manifest.json`에 **`partLabelsByIndex`** 가 있으면 성부 라벨 preset도 복원됩니다. **1단계 full**만 처음 돌릴 때는 PDF 1차 추출(미분류)로 시작합니다.
