...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | 거리 드롭다운은 XML 재빌드·OSMD 재로드 없이 VexFlow `.vf-modifiers`를 오선 칸만큼 이동. `npx tsx _smoke/test_articulation_pending_preview.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
