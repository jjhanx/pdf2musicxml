...
| tenuto·accent 거리 미리보기 | **복수 표:** 네이티브 글리프 숨김 + notehead 기준 SVG overlay(표별 N칸). UI: `Tenuto 거리` / `추가할 표 거리`. 검증: `npx tsx _smoke/test_dual_articulation_distance.ts` · `npx tsx _smoke/test_dual_art_ui_path.ts` · `npx tsx _smoke/test_dual_art_add_accent_overlay.ts` | 2칸·5칸 → overlay Y 독립. 「MXL에 반영」 후에도 hint+pending으로 유지 |
