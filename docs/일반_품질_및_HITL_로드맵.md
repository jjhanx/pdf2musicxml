...
| 여러 음에 화음 일괄 추가 시 전부 첫 음에 붙음 · 기존 음은 화음 1개씩만 | `leaderPitch*`+`chordMembers[]` · 같은 마디 `insertChordMember`는 큰 `leaderNoteIndex`부터 적용. 기존/신규 모두 「+ 화음 줄」. 편집기 목록은 리더 직후 화음 묶음·`+화음[…]` 표시. `python _smoke/test_multi_leader_chord_batch.py` | 마디 편집에서 리더마다 화음 여러 개 추가 후 「MXL에 반영」 |
