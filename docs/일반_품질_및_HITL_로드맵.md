...
| tenuto·accent 등이 음표 위/아래·이음줄과 겹침 | **mf와 다름:** OSMD는 direction `default-y`만 씀. Accent는 VexFlow가 XML `default-y` 무시. **즉시 미리보기는 항상 `.vf-modifiers` SVG transform** (`draw()` 성공으로 폴백을 건너뛰면 브라우저만 깨짐). `npx tsx _smoke/test_articulation_pending_preview.ts` | 마디 편집 거리. 저장 MXL은 「MXL에 반영」 |
