...
from fix_audiveris_mxl import _general_resolve_overfull_measure, _iter_measures_with_timing, mxl_ns_uri, _voice_groups, _note_duration
    ...
    for m, div, exp in _iter_measures_with_timing(p, ns):
        if m.get('number') == '29':
            for (staff, voice), groups in _voice_groups(m, ns).items():
                ...
                print(f'Staff {staff} Voice {voice}: total={total} expected={exp}')
                if total > exp:
                    overflow = total - exp
